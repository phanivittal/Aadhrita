...
<body style="background-image: linear-gradient(to right, #0F2027, #203A43,#2C5364);">
	...
	// following files need to be included
        ...
        include '../../../../db/db.php';
        $conn=mysqli_connect("localhost","root","","aadhrita");
        $paytmChecksum = "";
        $paramList = array();
	...
	if($isValidChecksum == "TRUE") {
        ...
        if($row['etype'] == "CULTURAL" && $_POST["STATUS"] == "TXN_SUCCESS" ) {
            echo "
            <div class = 'container' style = ' padding-top: 100px; ' >
            <div class = 'jumbotron'>
            ";
            $stl="select * from culturalregistrations where aadhritaID='DUMMY'";
            $dummy=mysqli_query($conn,$stl);
            $g=mysqli_fetch_assoc($dummy);
            print_r($g);
            ...
            if ($params[2]=='Txn Success') { 
                ...
                for( $j = 0 ; $j < $tsize ; $j++ ){
                    if ($j==0) {
                      $sql2="INSERT INTO `culturalregistrations`(`teamToken`, `eventID`, `aadhritaID`, `name`, `email`, `role`, `teamSize`) VALUES('$teamToken','$eventId','$aad3id[$j]','$uname[$j]','$emails[$j]','lead','$tsize')";
                   //   echo "<br>".$teamToken." ".$eventId." ".$aad3id[$j]." ".$emails[$j]." ".$tsize."<br>";
                  }
                  else{
                     $sql2="INSERT INTO `culturalregistrations`(`teamToken`, `eventID`, `aadhritaID`, `name`, `email`, `role`, `teamSize`) VALUES('$teamToken','$eventId','$aad3id[$j]','$uname[$j]','$emails[$j]','lead','$tsize')";
                  }
