 ...
 <form action="forget.php" method="POST" enctype="multipart/form-data" >
    <div class="container">
      ...
      <div class="form-group">
        ...
        <input type="text" class="form-control" name="email" placeholder="Registered EmailID"  required >
      </div>
    
     
      <div class="form-group"  >
        <label>Enter aad3 :</label> <i class="fas fa-lock"></i>
        <input type="text" class="form-control" name="aad3" placeholder="Enter Aadhrita ID" required>
      </div>
      <div class="form-group" id="submit" >
        <input type="submit" class="btn btn-outline-success" name="submit" value="Confirm" onclick="return validate()">
...
<script>
   var k=document.getElementByName('email').value;
   var L=document.getElementByName('aad3').value;

function validate(){
  if(k==""&&L==""){
    alert("email and aadhrita should not be empty");
    return false;
  }
  else{
    return true;
...
<?php
  ...
  function getpass(){
    ...
    $aad3=$_POST['aad3'];
    require '../db/db.php';
    $sql = "SELECT password FROM `registrations` where email='$email' && aadhritaID='$aad3'";
          $retval = mysqli_query($conn,$sql);
         if( $result = mysqli_fetch_assoc($retval)){
            $password=$result['password'];
          echo "<script type='text/javascript'>alert('$password');</script>";
         }else{
            echo "<script type='text/javascript'>alert('NOT FOUND ENTER VALUES IN GOOGLE FORM');location.href='../../index.php';</script>";
         }
